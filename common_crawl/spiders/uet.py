...
class UETConfig:
  ...
  def __init__(self):
    self.title_css = [".single-content-title",\
     "#content strong"]
    self.content_css = [".single-post-content-text",\
     ".single-page-content"]
...
class UETSpider(CrawlSpider):
    name = "uet"
    allowed_domains = ["uet.edu.edu.vn"]
    start_urls = ["https://uet.vnu.edu.vn/"]
    rules = [
      Rule(LinkExtractor(allow=r"https://.*uet\.vnu\.edu\.vn/.*"),\
       callback='parse_item', follow=True),
    ]

    def __init__(self, *a, **kw):
      super(UETSpider, self).__init__(*a, **kw)
      self.config = UETConfig()
      print("Init")
